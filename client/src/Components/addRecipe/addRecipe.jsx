...
import { addRecipe, getDiets } from "../../Actions/Actions";
...
function control(input){
    ...
    if(!input.name){
        ...
        error.HS = 'A Health Score is required!'
    }
    else if (!input.steps) {
...
export default function RecipeCreated(){
    ...
    function handleSelectDiet(e){
        ...
        console.log(typeof input.diets);
    }

    function handleSumbit(e){
        e.preventDefault();
        if(input.name && input.resume && input.img && input.HS && input.steps){
    ...
    return (
        ...
        <div className= 'data'>
            <h2 id ="titulo">Complete The Data</h2>
            <form onSubmit= {e => handleSumbit(e)}>
                ...
                <div>
                    ...
                    <input className='inputs' type="url"  value={input.img} name='img'
                    onChange={(e) =>handleInputsChange(e)}/>
                    {
                        errors.img &&(
                ...
                <div>
                    ...
                    <input id='HS' type="number" value={input.HS} name='HS' 
                    onChange={(e) =>handleInputsChange(e)}/>
                </div>
                <div>
                ...
                <select className="select" onChange={(e) => handleSelectDiet(e)}>
                    {
                        diets?.map((dieta) => {
                            return (
                                    <option key={dieta.id} value={dieta.name}>{dieta.name[0].toUpperCase()+ dieta.name.slice(1)}</option>
                        )
                        })
                    }
                </select>
                <h1> </h1>
                    <button id="addRecipe" type='submit'>¡ADD!</button>
                
            </form>
                    {input.diets.map(el => {
                        return <span key={el} className='diets'>
